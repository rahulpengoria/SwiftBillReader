   ...
   ///   - completionBlock: Returns if the URL is handled/not handled
    public func handleRouting(for url: URL,from source: DeeplinkSource = .externalLink, additionalParams: [String:Any]? = nil,with completionBlock:((Bool)->Void)? = nil) {
        ...
        if let jsUrl = self.hasValidJavaScriptRouter(for: url.absoluteString),
            self.validateAndProceed(with: jsUrl, originalUrl: url, from: source, additionalParams: additionalParams) {
            completionBlock?(true)
        } else {
            if self.validateAndProceed(with: url, originalUrl: url, from: source, additionalParams: additionalParams) {
                completionBlock?(true)
            } else {
                self.getAlternateLink(for: url) { (redirectUrl) in
                    if let redirectUrl = redirectUrl {
                        self.handleRouting(for: redirectUrl, from: source, additionalParams: additionalParams, with: completionBlock)
                    } else {
    ...
    private func validateAndProceed(with url: URL, originalUrl: URL,from source: DeeplinkSource, additionalParams: [String:Any]? = nil) -> Bool {
        ...
        router.originalUrl = originalUrl
        router.deeplinkSource = source
        router.additionalParams = additionalParams
