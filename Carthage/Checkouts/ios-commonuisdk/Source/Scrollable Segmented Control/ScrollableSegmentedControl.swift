...
public enum ScrollableSegmentedControlSegmentStyle: Int {
    ...
    /// Gets the class which is used to display the cell
    ///
    /// - Returns: AnyClass, which can be of the following types:
    ///  TextOnlySegmentCollectionViewCell, ImageOnlySegmentCollectionViewCell, ImageOnTopSegmentCollectionViewCell, ImageOnLeftSegmentCollectionViewCell
    public func getCellType() -> AnyClass {
        switch self {
        case .textOnly:
            return ScrollableSegmentedControl.TextOnlySegmentCollectionViewCell.self
        case .imageOnly:
            return ScrollableSegmentedControl.ImageOnlySegmentCollectionViewCell.self
        case .imageOnTop:
            return ScrollableSegmentedControl.ImageOnTopSegmentCollectionViewCell.self
        case .imageOnLeft:
            return ScrollableSegmentedControl.ImageOnLeftSegmentCollectionViewCell.self
        }
    }
}
...
/**
 Conform SegmentControlViewDecorator Delegate
 */
public protocol SegmentControlViewDecoratorDelegate {
    /**
     Confirm SegmentControlViewDecorator Delegate
     */
    
    /// Modify cell before it is returned
    ///
    /// - Parameters:
    ///   - cell: cell before it is returned by cellForItemAt
    ...
    ///
    /// - Parameters:
    ///   - cell: The cell which has been deselected
...
public protocol TabDelegateCustomXLPager {
    func didEndScrollingSegmentedControl(_ scrollView: UIScrollView)
    func didFinishScrollingAnimationSegmentedControl(_ scrollView: UIScrollView)
}
...
@objc public class ScrollableSegmentedControl: UIControl {
    ...
    public var viewDecoratorDelegate: SegmentControlViewDecoratorDelegate?
    public var tabDelegate: TabDelegateCustomXLPager?
    /**
            ...
            if selectedSegmentIndex >= 0 {
                ...
                if let indexPath = collectionView?.indexPathsForSelectedItems?.first {
                    if let cell = collectionView?.cellForItem(at: indexPath) {
                        viewDecoratorDelegate?.didDeselect(cell: cell, indexPath: indexPath)
                    }
                }
                
                collectionView?.selectItem(at: indexPath, animated: true, scrollPosition: scrollPossition)
                if let cell = collectionView?.cellForItem(at: indexPath) {
                    viewDecoratorDelegate?.didSelect(cell: cell, indexPath: indexPath)
                }
    ...
    final private class CollectionViewController : NSObject, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout{
        ...
        public func scrollViewDidScroll(_ scrollView: UIScrollView) {
            segmentedControl.tabDelegate?.didEndScrollingSegmentedControl(scrollView)
        }
        
        
        func scrollViewDidEndScrollingAnimation(_ scrollView: UIScrollView) {
            // setting this selectedSegmentIndex again sets the selectedTab's properties(text color)
            segmentedControl.selectedSegmentIndex = segmentedControl.selectedSegmentIndex
            segmentedControl.tabDelegate?.didFinishScrollingAnimationSegmentedControl(scrollView)
        }
        ...
        public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
            ...
            segmentedControl.viewDecoratorDelegate?.setup(cell: segmentCell, indexPath: indexPath)
            return segmentCell
        }
        ...
        fileprivate func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
            ...
            segmentedControl.viewDecoratorDelegate?.didSelect(cell: cell, indexPath: indexPath)
        }
        ...
        fileprivate func collectionView(_ collectionView: UICollectionView, didDeselectItemAt indexPath: IndexPath) {
            ...
            segmentedControl.viewDecoratorDelegate?.didDeselect(cell: cell, indexPath: indexPath)
        }
    ...
    public class BaseSegmentCollectionViewCell: UICollectionViewCell {
    ...
    public class TextOnlySegmentCollectionViewCell: BaseSegmentCollectionViewCell {
        let titleLabel = UILabel()
        
        override public var contentColor:UIColor? {
            didSet {
                titleLabel.textColor = (contentColor == nil) ? BaseSegmentCollectionViewCell.defaultTextColor : contentColor!
            }
        }
        
        override public var selectedContentColor:UIColor? {
            didSet {
    ...
    public class ImageOnlySegmentCollectionViewCell: BaseSegmentCollectionViewCell {
    ...
    public class BaseImageSegmentCollectionViewCell: BaseSegmentCollectionViewCell {
        let titleLabel = UILabel()
        let imageView = UIImageView()
    ...
    public class ImageOnTopSegmentCollectionViewCell: BaseImageSegmentCollectionViewCell {
        override func configure(){
            super.configure()
    ...
    public class ImageOnLeftSegmentCollectionViewCell: BaseImageSegmentCollectionViewCell {
        override func configure(){
            super.configure()
